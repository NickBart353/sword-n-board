...
func _load_player_items():
	...
	player_items = item_dict.get("inventory").map(ItemManager.get_item_from_existing_itemdata)
...
func _load_debug_items(skip: bool = false) -> void:
	if skip:
		return
	player_items.append_array(ItemManager.get_all_upgrade_types_for_all_items())
	return
