...
func _place_room(x_loc: int, z_loc: int) -> Node3D:
	var room_fits = false
	var room_instance
	while not room_fits:
		room_instance = room_scenes[randi_range(0, room_scenes.size()-1)].instantiate()
		if room_instance.dimensions["x"] == 1 and room_instance.dimensions["y"] == 1:
			room_fits = true
		else:
			for x in range(room_instance.dimensions["x"]):
				for y in range(room_instance.dimensions["y"]):
					if ((x_loc+x < dungeon_width -1) and (rooms[x_loc+x][z_loc] == null)) and ((z_loc+y < dungeon_height -1) and (rooms[x_loc][z_loc+y] == null)):
						room_fits = true
	
	room_instance.transform.origin = Vector3(x_loc * room_size.x, 0, z_loc * room_size.z)
